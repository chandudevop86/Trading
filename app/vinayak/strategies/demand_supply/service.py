...
def summarize_structure_metrics(
    ...
    summary["totals"] = {
        "zones_detected": int(len(zone_list)),
        "trade_candidates": int(len(trade_list)),
        "rejections": int(len(reject_list)),
    }
    return summary
...
def _candles_to_frame(candles: Sequence[Candle] | pd.DataFrame) -> pd.DataFrame:
        ...
        signals.append(
            StrategySignal(
                ...
                metadata={
                    ...
                    "zone_id": str(trade["zone_id"]),
                    "zone_type": str(trade["zone_type"]),
                    "zone_low": float(trade["zone_low"]),
                    "zone_high": float(trade["zone_high"]),
