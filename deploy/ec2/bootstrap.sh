...
if [ ! -f "$APP_ENV_FILE" ]; then
  sudo cp "$APP_DIR/deploy/config/trading.env.example" "$APP_ENV_FILE"
  sudo chmod 600 "$APP_ENV_FILE"
fi

sudo cp "$APP_DIR/deploy/systemd/trading-ui.service" /etc/systemd/system/trading-ui.service
sudo cp "$APP_DIR/deploy/systemd/trading-daemon.service" /etc/systemd/system/trading-daemon.service
