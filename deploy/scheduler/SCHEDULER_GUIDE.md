...
```bash
cd /opt/intratrade
chmod +x deploy/scheduler/run_auto_backtest.sh
```
...
Add this line:
```cron
*/15 * * * * REPO_DIR=/opt/intratrade SYMBOL=RELIANCE.NS INTERVAL=5m PERIOD=5d EXEC_SYMBOL=RELIANCE /opt/intratrade/deploy/scheduler/run_auto_backtest.sh >> /opt/intratrade/data/cron_backtest.log 2>&1
```
