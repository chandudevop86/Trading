...
def test_indicator_returns_standardized_signal_shape() -> None:
    candles = [
    ...
    signals = run_indicator_strategy(candles=candles, symbol='^NSEI')
    assert signals
    signal = signals[0]
    assert signal.strategy_name == 'Indicator'
