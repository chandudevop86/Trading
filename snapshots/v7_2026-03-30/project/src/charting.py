...
def build_live_market_chart(candles: pd.DataFrame, output_rows: list[dict[str, object]] | None = None) -> alt.Chart:
    ...
    price_panel = alt.layer(*layers).properties(height=210)
    ...
    return alt.vconcat(price_panel, volume, spacing=4).resolve_scale(x="shared").configure_view(stroke=None).configure(background="#0b1220")
