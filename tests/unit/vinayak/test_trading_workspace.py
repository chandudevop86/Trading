...
def test_run_live_trading_analysis_notification_event_omits_credentials(monkeypatch, tmp_path: Path) -> None:
    published: list[tuple[str, dict[str, object], str]] = []

    class _StubBus:
        def publish(self, name: str, payload: dict[str, object], *, source: str) -> bool:
            published.append((name, payload, source))
            return True

    monkeypatch.setattr(service, 'build_message_bus', lambda: _StubBus())
    monkeypatch.setattr(service, 'fetch_live_ohlcv', lambda **kwargs: [{
        'timestamp': '2026-03-24 09:15:00',
        'open': 100.0,
        'high': 102.0,
        'low': 99.0,
    ...
    monkeypatch.setattr(service, 'attach_lots', lambda rows, **kwargs: rows)
    monkeypatch.setattr(service, '_build_report_artifacts', lambda result: {
        'json_report': {'local_path': str(tmp_path / 'report.json')},
        'summary_report': {'local_path': str(tmp_path / 'summary.txt')},
    })
    monkeypatch.setattr(service, 'publish_active_alerts', lambda **kwargs: 0)
    monkeypatch.setattr(service, 'send_telegram_message', lambda token, chat_id, text: {'ok': True})
    monkeypatch.setattr(service, 'run_strategy_workflow', lambda context, **kwargs: [
        {
            'strategy': 'BTST',
            'symbol': context.symbol,
            'side': 'BUY',
    ...
    service.run_live_trading_analysis(
        symbol='^NSEI',
        interval='5m',
        period='1d',
        strategy='BTST',
        ...
        mtf_max_trades_per_day=3,
        send_telegram=True,
        telegram_token='request-token',
        telegram_chat_id='request-chat',
        paper_log_path=str(tmp_path / 'paper.csv'),
    )

    notification_events = [item for item in published if item[0] == 'notification.requested']
    assert len(notification_events) == 1
    payload = notification_events[0][1]
