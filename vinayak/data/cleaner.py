...
class OHLCVValidationError(ValueError):
    """Raised when raw market data cannot be normalized into a safe OHLCV frame."""
...
def _rename_columns(frame: pd.DataFrame) -> pd.DataFrame:
    out = frame.copy()
    out.columns = [COLUMN_ALIASES.get(_normalize_column_name(column), _normalize_column_name(column)) for column in frame.columns]
    return out
...
def coerce_ohlcv(df: Any, config: CleanerConfig | None = None) -> pd.DataFrame:
    """Normalize raw OHLCV data into a strict, strategy-safe dataframe."""
