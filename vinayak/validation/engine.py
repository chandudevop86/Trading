...
def validate_trade(setup: dict[str, Any], candles: pd.DataFrame, config: ValidationConfig | None = None) -> dict[str, Any]:
    """Validate a generic trade candidate against cleaned candles and return an execution gate."""
    ...
    try:
        ...
        metrics["structure_clarity"] = round(
            _safe_float(setup.get("structure_clarity", setup.get("structure_score", max(0.0, 10.0 - float(metrics["chop_score"]))))),
            4,
        )
        ...
        if rr_ratio < cfg.rr_threshold:
            reasons.append("bad_rr")
        if cfg.require_vwap_alignment and not vwap_alignment:
            reasons.append("no_vwap_alignment")
        if cfg.require_trend_alignment and not trend_alignment:
            reasons.append("trend_misaligned")
        if float(metrics["atr_pct"]) < cfg.min_volatility_pct:
            reasons.append("low_volatility")
        if float(metrics["chop_score"]) > cfg.max_chop_score:
